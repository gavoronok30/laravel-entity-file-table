...
```
        \Gavoronok30\LaravelEntityFileTable\Providers\EntityFileTableServiceProvider::class,
```
...
```
php artisan vendor:publish --provider="Gavoronok30\LaravelEntityFileTable\Providers\EntityFileTableServiceProvider" --tag=config
```
...
class CustomFile extends \Gavoronok30\LaravelEntityFileTable\Model\FileModel
{
}
```
...
...
// remove
'model' => \Gavoronok30\LaravelEntityFileTable\Model\FileModel::class,
// add
...
After added custom file, use "CustomFile" model everywhere, instead of "\Gavoronok30\LaravelEntityFileTable\Model\FileModel"

## Helpers

#### Gavoronok30\LaravelEntityFileTable\Helpers\FileHelper

```
// $file - \Gavoronok30\LaravelEntityFileTable\Model\FileModel
// generate url from file
return \Gavoronok30\LaravelEntityFileTable\Helpers\FileHelper::urlFile($file);

// generate url from data
return \Gavoronok30\LaravelEntityFileTable\Helpers\FileHelper::url('file.jpg', 'public');
```
...
{
    return $this->morphOne(\Gavoronok30\LaravelEntityFileTable\Model\FileModel::class, 'model', 'model_type', 'model_id');
}
```
...
{
    return $this->morphMany(\Gavoronok30\LaravelEntityFileTable\Model\FileModel::class, 'model', 'model_type', 'model_id');
}
```
...
```
$service = app(\Gavoronok30\LaravelEntityFileTable\EntityFileTableServiceInterface::class);
```
...
```
// use
use Gavoronok30\LaravelEntityFileTable\EntityFileTableServiceInterface;
// constructor
...
Get data of upload file, **return \Gavoronok30\LaravelEntityFileTable\FileData**
...
Get file model from upload file, **return \Gavoronok30\LaravelEntityFileTable\Model\FileModel**

```
// $fileData - \Gavoronok30\LaravelEntityFileTable\FileData
$file = $service->getFile($fileData);
```

Get file model from link, **return \Gavoronok30\LaravelEntityFileTable\Model\FileModel**
...
// $file - return \Gavoronok30\LaravelEntityFileTable\Model\FileModel
$service->save($file);
...
Get file by ID, **return \Gavoronok30\LaravelEntityFileTable\Model\FileModel**

```
$file = $service->getFileById(1);
```

Get files by ID, **return \Gavoronok30\LaravelEntityFileTable\Model\FileModel[] | Illuminate\Database\Eloquent\Collection**
...
// $file1, $file2, ... - \Gavoronok30\LaravelEntityFileTable\Model\FileModel
// $files - Collection
...
```
$file = \Gavoronok30\LaravelEntityFileTable\Model\FileModel::find(1);
